...
export default function DictPage() {
  ...
  function handleSave(id: number) {
    const suggestions = form.suggestions.split(',').map(s => s.trim()).filter(Boolean)
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      <table className="w-full border-collapse">
        <thead>
          <tr>
            <th className="text-left px-3 py-2 border-b-2 border-gray-300 bg-gray-50 text-xs font-semibold text-gray-600">错误词</th>
            <th className="text-left px-3 py-2 border-b-2 border-gray-300 bg-gray-50 text-xs font-semibold text-gray-600">建议修改</th>
            <th className="text-left px-3 py-2 border-b-2 border-gray-300 bg-gray-50 text-xs font-semibold text-gray-600">级别</th>
            <th className="text-left px-3 py-2 border-b-2 border-gray-300 bg-gray-50 text-xs font-semibold text-gray-600">操作</th>
          </tr>
        </thead>
        <tbody>
          {dict.map(entry => (
              ...
              {editingId === entry.id ? (
                <>
                  <td className="px-3 py-2 border-b border-gray-200">
                    <input className="w-full px-2 py-1 border border-gray-300 rounded text-sm" value={form.word}
                      onChange={e => setForm({ ...form, word: e.target.value })} />
                  </td>
                  <td className="px-3 py-2 border-b border-gray-200">
                    <input className="w-full px-2 py-1 border border-gray-300 rounded text-sm" value={form.suggestions}
                      onChange={e => setForm({ ...form, suggestions: e.target.value })} />
                  </td>
                  <td className="px-3 py-2 border-b border-gray-200">
                    <select className="px-2 py-1 border border-gray-300 rounded text-sm bg-white" value={form.level}
                      onChange={e => setForm({ ...form, level: Number(e.target.value) })}>
                      <option value={1}>拼写错误</option>
                      <option value={2}>建议修改</option>
                      <option value={3}>疑似错误</option>
                  ...
                  <td className="px-3 py-2 border-b border-gray-200">
                    <button className="px-2.5 py-1 rounded border border-gray-300 bg-white text-xs cursor-pointer mr-1 hover:bg-gray-50"
                      onClick={() => handleSave(entry.id)}>保存</button>
                    <button className="px-2.5 py-1 rounded border border-gray-300 bg-white text-xs cursor-pointer hover:bg-gray-50"
                ...
                <>
                  ...
                  <td className="px-3 py-2 border-b border-gray-200 text-sm">{entry.suggestions.join(', ')}</td>
                  <td className="px-3 py-2 border-b border-gray-200 text-sm">{levelLabel[entry.level]}</td>
                  <td className="px-3 py-2 border-b border-gray-200">
                    <button className="px-2.5 py-1 rounded border border-gray-300 bg-white text-xs cursor-pointer mr-1 hover:bg-gray-50"
                      onClick={() => handleEdit(entry)}>编辑</button>
                    <button className="px-2.5 py-1 rounded border border-red-300 bg-white text-xs cursor-pointer text-red-600 hover:bg-red-50"
                      onClick={() => removeEntry(entry.id)}>删除</button>
                  </td>
                </>
              )}
            </tr>
