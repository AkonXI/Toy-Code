...
export function ensureUser(phone: string): number {
  ...
  const result = db
    .prepare(
      "INSERT INTO users (phone, nickname, created_at, updated_at) VALUES (?, ?, ?, ?)",
    )
...
export function getInitialPrompt(id: string): string | null {
  ...
  const row = db
    ...
    .get(id) as { initial_prompt: string | null } | undefined;
...
export function getConversationChunks(
...
export async function buildHistoryPrompt(
  conversationId: string,
): Promise<string> {
  const db = getDatabase();
...
export function getConversationDocs(
  conversationId: string,
): string[] {
  const db = getDatabase();
  const rows = db
...
export function setConversationChunks(
  conversationId: string,
  chunks: Chunk[],
) {
  const db = getDatabase();
  ...
  db.prepare("DELETE FROM chunks WHERE conversation_id = ?").run(
    conversationId,
  );
...
export function setConversationChunksWithTypes(
  conversationId: string,
  chunks: { pageContent: string; metadata: Record<string, unknown>; docType?: "resume" | "reference"; scope?: string; refCategory?: string }[],
  refId?: number,
) {
  const db = getDatabase();
  const now = Date.now();
...
export function deleteChunksByRefId(
  conversationId: string,
  refId: number,
): number {
  const db = getDatabase();
...
export function getConversationChunksWithTypes(
  conversationId: string,
): TypedChunk[] {
  const db = getDatabase();
  const rows = db
    ...
    .all(conversationId) as { page_content: string; metadata: string; doc_type: string; ref_id: number | null; scope: string | null; ref_category: string | null }[];
  return rows.map((r) => ({
    pageContent: r.page_content,
    metadata: JSON.parse(r.metadata),
...
export function getUserByPhone(phone: string): User | null {
  ...
  const row = db
    .prepare(
      "SELECT id, phone, nickname, created_at, updated_at FROM users WHERE phone = ?",
    )
