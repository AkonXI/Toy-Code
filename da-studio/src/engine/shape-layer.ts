...
export class ShapeLayer {
  ...
  hitTest(pixelX: number, pixelY: number): number {
    const img = this._viewport.toImage(pixelX, pixelY)
    ...
    for (let i = len - 1; i >= 0; i--) {
      ...
      if (s.type === 'rect') {
        ...
        const p = this._viewport.toCanvas(s.x, s.y)
  ...
  vertexHitTest(pixelX: number, pixelY: number): VertexHit | null {
    ...
    for (let i = len - 1; i >= 0; i--) {
      ...
      for (let j = 0; j < pts.length; j++) {
        const p = this._viewport.toCanvas(pts[j].x, pts[j].y)
  ...
  _hitPolyline(pixelX: number, pixelY: number, points: Point[]): boolean {
    for (let j = 0; j < points.length; j++) {
      const p = this._viewport.toCanvas(points[j].x, points[j].y)
  ...
  _hitSegment(px: number, py: number, a: Point, b: Point): boolean {
    const pa = this._viewport.toCanvas(a.x, a.y)
    const pb = this._viewport.toCanvas(b.x, b.y)
    const dx = pb.x - pa.x,
      dy = pb.y - pa.y
  ...
  _drawShapeLabels(): void {
    ...
    this.shapes.forEach((s, i) => {
      ...
      const p = this._viewport.toCanvas(anchor.x, anchor.y)
  ...
  _paintRect(s: RectShape, highlight: boolean): void {
    ...
    if (s.current) this._paintHandles(cw, ch, c)
    this.ctx.restore()
  }
  ...
  _paintPoint(s: PointShape, i: number, highlight: boolean): void {
    ...
    this.ctx.fillStyle = s.current ? c.fill : highlight ? c.fillHover : c.stroke
    this.ctx.fill()
  ...
  _paintPolyline(s: PolylineShape, _i: number, highlight: boolean): void {
    ...
    if (pts.length > 1) {
      this.ctx.beginPath()
      const p0 = this._viewport.toCanvas(pts[0].x, pts[0].y)
  ...
  _paintPolygon(s: PolygonShape, _i: number, highlight: boolean): void {
    ...
    const c = this._groups[s.group] || this._defaultGroup
    if (pts.length === 1) {
      const p = this._viewport.toCanvas(pts[0].x, pts[0].y)
      this.ctx.beginPath()
    ...
    this.ctx.closePath()
    if (highlight) this.ctx.fillStyle = c.fillHover
    else if (s.current) this.ctx.fillStyle = c.fill
    else this.ctx.fillStyle = 'rgba(0,0,0,0.04)'
    this.ctx.fill()
    this.ctx.strokeStyle = c.stroke
    this.ctx.lineWidth = 2
