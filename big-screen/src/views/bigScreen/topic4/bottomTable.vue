                ...
                <ul>
                    <li class="table-body-item" v-for="(item, index) in tableData" :key="index">
                        <template v-for="(col, ii) in headerColunm">
                            <div class="col-item" :key="ii" v-if="col.key === 'index'" :style="{width:col.width,textAlign:col.align}">{{index+1}}</div>
                            <div class="col-item oprate" :key="ii" v-else-if="col.key === 'oprate'" :style="{width:col.width,textAlign:col.align}" @click="goView(item)">查看</div>
                            <div class="col-item item-name" v-else-if="col.key === 'name'" :key="ii" :title="item.name" :style="{width:col.width,textAlign:col.align}">
                                <template>
                                    <img class="item-icon" v-if="item.type == '2'" src="@/assets/bigScreen/risk-type-warn.png" alt="">
