...
import { addFileToConversation, getConversationDocsByType } from '../src/storage/file-manager'
...
describe('Chunk Classification', () => {
  ...
  it('should store ref_id on chunks for precise file tracking', async () => {
    ...
    const db = getDatabase()
    const row = db
      .prepare('SELECT ref_id FROM chunks WHERE conversation_id = ? AND page_content = ?')
...
describe('Initial Prompt', () => {
  it('should create conversation with initial_prompt', async () => {
    ...
    const db = getDatabase()
    const row = db.prepare('SELECT initial_prompt FROM conversations WHERE id = ?').get(convId) as {
