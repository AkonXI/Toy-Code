...
export default function EditorPage() {
  ...
  const handleSelectError = useCallback((errorId: string) => {
    ...
    const span = editorRef.current?.getBody().querySelector(`[data-error-id="${errorId}"]`) as HTMLElement | null
    if (span) {
    ...
    <div className="max-w-[1200px] mx-auto p-6">
      ...
      <div className="flex gap-4 items-start">
        <div className="flex-1 min-w-0 relative">
          {!editorReady && (
            <div className="absolute inset-0 flex items-center justify-center bg-white rounded border border-gray-200 z-10" style={{ height: 500 }}>
          ...
          <Editor
            ...
            onInit={(_evt, editor) => { editorRef.current = editor; setEditorReady(true) }}
...
<p>𠀀𪚥㐀<i>需</i>要∑∫√<b>登</b>陆∞²³©®<em>必</em>须</p>`}
            init={{
              height: 500,
              language: 'zh_CN',
              toolbar: 'spellcheck clearspell | undo redo | bold italic underline | forecolor backcolor',
              menubar: false,
              plugins: '',
              setup: (editor: any) => {
                apiRef.current = setupTinyMCESpellCheck(
                  editor,
                  () => getErrorMap(),
                  (list: ErrorTrayItem[]) => { setErrorList(list); setSelectedId(null) },
                  (replacedId: string) => setErrorList(prev => prev.filter(e => e.errorId !== replacedId)),
