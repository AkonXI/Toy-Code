...
export function sectionsToContentArray(
...
function parseLineToPdfmake(
  ...
  if (listMatch) {
    ...
    return {
      text: [
        { text: bullet, fontSize: 10, font: "SourceHan" },
...
function parseStyle(styleStr: string): Record<string, any> {
  ...
  for (const rule of styleStr.split(";")) {
    ...
    else if (pdfKey === "decoration")
      result[pdfKey] = v === "underline" ? "underline" : v;
...
function parseInline(text: string): Record<string, any>[] {
  ...
  return segments.length > 0
    ? segments
    : [{ text, fontSize: 10, font: "SourceHan", color: "#333" }];
}
...
function parseInlineForHeading(
  text: string,
  fontSize: number,
  bold: boolean,
): Record<string, any> {
...
export function parseAIContent(text: string): Record<string, any>[] {
  ...
  if (trimmed.startsWith("[") || trimmed.startsWith("[")) {
    try {
      ...
      console.log(
        "[pdfmaker] JSON parse failed, falling back to markdown parser",
      );
    }
  }
  ...
  if (bracketMatch) {
    try {
      ...
      console.log(
        "[pdfmaker] JSON extraction failed, falling back to markdown parser",
      );
    }
  }
