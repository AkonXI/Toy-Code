...
export async function triggerAutoSummary(conversationId: string): Promise<void> {
  ...
  const unsummarized = db
    .prepare(
      "SELECT COUNT(*) as cnt FROM messages WHERE conversation_id = ? AND summarized = 0",
    )
  ...
  const prevSummaryRow = db
    .prepare("SELECT summary FROM conversation_summaries WHERE conversation_id = ? ORDER BY end_message_id DESC LIMIT 1")
...
async function compressSummaries(conversationId: string): Promise<void> {
  ...
  const summaries = db
    ...
    .all(conversationId) as { id: number; summary: string; message_count: number; start_message_id: number; end_message_id: number }[];
