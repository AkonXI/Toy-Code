...
function parseGroups(raw: string | null): any[] {
  if (!raw) return []
  try {
...
function buildGroupIdMap(groups: any[]): Map<string, string> {
  return new Map(groups.map((g, i) => [g.name || String(i + 1), g.id || String(i + 1)]))
}
function groupIdOf(m: Map<string, string>, name: string): string {
  return typeof name === 'string' ? m.get(name) || name : ''
}
function exportShape(s: any, m: Map<string, string>) {
  const { group, current, ...rest } = s
  return { ...rest, group_id: groupIdOf(m, group ?? '') }
}
function csv(v: unknown): string {
  return `"${(v == null ? '' : String(v)).replace(/"/g, '""')}"`
}
function parseShapes(raw: string): any[] {
  try {
...
export function registerExportIpcHandlers(): void {
  ipcMain.handle('annotation:export', async (_e, datasetId: number, format: string) => {
    const rows = getDatasetAnnotations(datasetId)
    ...
    if (format === 'json') {
      writeFileSync(
        ...
        JSON.stringify(
          rows.map((r) => ({
            image_md5: r.image_md5 || String(r.image_id),
            image_name: r.image_name,
            shapes: r.parsedShapes.map((s: any) => exportShape(s, groupMap)),
      ...
      for (const row of rows) {
        for (const shape of row.parsedShapes) {
          ...
          if (shape.type === 'rect')
            lines.push(
              `${base},${shape.x},${shape.y},${shape.w},${shape.h},${shape.rotation ?? 0},`
            )
          else if (shape.type === 'point') lines.push(`${base},${shape.x},${shape.y},,,,,`)
          else if ('points' in shape)
            lines.push(
              `${base},,,,,,${csv(shape.points.map((p: any) => `${p.x},${p.y}`).join(';'))}`
            )
        }
      }
