<template>
  <div class="max-w-[1160px] mx-auto p-5 font-sans text-gray-800">
    ...
    <div class="flex gap-3 items-start">
      ...
      >
        <div class="text-sm font-semibold px-4 py-3 border-b border-gray-100">
          标注列表
        </div>
        <div class="flex-1 overflow-y-auto p-2" ref="listRef">
          <div
            v-if="shapes.length === 0"
            class="text-gray-300 text-sm text-center mt-10"
          >
            暂无标注。<br />点击拖拽开始
          </div>
          <div
          ...
          >
            <template v-if="s.type === 'rect'">
              ...
              <span v-if="s.rotation" class="text-red-600"
            ...
            <template v-else-if="s.type === 'polyline'">
              ...
              <span v-if="!s.complete" class="text-orange-500 italic"
                >(编辑中)</span
              ><br />
              <span class="text-gray-500 font-mono text-[11px]"
...
<script setup lang="ts">
  ...
  const modeKeyMap = Object.fromEntries(
    MODE_LIST.map(m => [m.key, m.type]),
  ) as Record<string, ModeType>;

  const enabledSet = computed(() => new Set(props.enabledModes));
  function isModeEnabled(t: ModeType) { return enabledSet.value.has(t); }
